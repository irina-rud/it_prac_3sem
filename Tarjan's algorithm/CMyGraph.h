...
//    CEdgeIterator OutgoingEdgesEnd(const CVertexIterator&) const;
    ...
    void Tarjan(){
        ...
        int NextL = 0;
        map <TVertexID, int> L;
        map <TVertexID, int> low;
        for (auto i:vertexes){
            L.insert(make_pair(i, -1));
        }
        for (auto i : vertexes){
            if (L[i]==-1){
                this->StrongConnect(i, NextL, S, L, low);
            }
        }
    }

    void StrongConnect(TVertexID& v,int& NextL, vector <TVertexID>& S, map <TVertexID, int>& L, map <TVertexID, int>& low){
        L[v] = NextL;
        low.insert(make_pair(v,L[v]));
        NextL ++;
        S.push_back(v);
        if (master->links.find(v) != master->links.end()){
            for (auto i = master->links.find(v); i != master->links.end() && i->first == v ; i++ ){
        ...
        if (low[v] = L[v]){
            cout << "strong connecled component: ";
            for (auto i = S.end() - 1; *i != v; --i){
                    cout << *i <<" ";
                    S.pop_back();
            }
            cout << *i <<endl;
